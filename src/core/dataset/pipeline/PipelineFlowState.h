...
/// Utility class that is used to reference a particular data object in a PipelineFlowState
...
public:
	...
	void removeObject(const DataObject* dataObj) { replaceObject(dataObj, nullptr); }

	/// \brief Removes a data object from this state.
	void removeObjectByIndex(int index);
		
	/// \brief Returns the list of data objects stored in this flow state.
	const std::vector<const DataObject*>& objects() const { 
		OVITO_STATIC_ASSERT(sizeof(StrongDataObjectRef) == sizeof(const DataObject*));
		OVITO_STATIC_ASSERT(sizeof(std::vector<StrongDataObjectRef>) == sizeof(std::vector<const DataObject*>));
		return reinterpret_cast<const std::vector<const DataObject*>&>(_objects);
	}
