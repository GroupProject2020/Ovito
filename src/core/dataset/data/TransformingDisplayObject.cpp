...
#include "TransformingDisplayObject.h"

namespace Ovito { OVITO_BEGIN_INLINE_NAMESPACE(ObjectSystem) OVITO_BEGIN_INLINE_NAMESPACE(Scene)

IMPLEMENT_OVITO_CLASS(TransformingDisplayObject);

/******************************************************************************
* Constructor.
******************************************************************************/
TransformingDisplayObject::TransformingDisplayObject(DataSet* dataset) : DisplayObject(dataset)
{
}
...
******************************************************************************/
Future<PipelineFlowState> TransformingDisplayObject::transformData(TimePoint time, DataObject* dataObject, PipelineFlowState&& flowState, const PipelineFlowState& cachedState, ObjectNode* contextNode) 
{
	// Check if the cache state already contains a transformed data object that we
	...
	for(DataObject* o : cachedState.objects()) {
		if(TransformedDataObject* transformedDataObject = dynamic_object_cast<TransformedDataObject>(o)) {
			if(transformedDataObject->sourceDataObject() == dataObject && transformedDataObject->displayObject() == this && transformedDataObject->generatorDisplayObjectRevision() == revisionNumber()) {
	...
	return future.then_future(executor(), [this, inputData = std::move(inputData)](Future<PipelineFlowState> future) mutable {
		try {
			...
			OVITO_ASSERT_MSG(false, "TransformingDisplayObject::transformData()", "Caught an unexpected exception type during display object data transformation.");
