...
DEFINE_PROPERTY_FIELD(VoronoiAnalysisModifier, outputPolyhedra);
...
SET_PROPERTY_FIELD_LABEL(VoronoiAnalysisModifier, computeBonds, "Generate nearest neighbor bonds");
SET_PROPERTY_FIELD_LABEL(VoronoiAnalysisModifier, outputPolyhedra, "Visualize Voronoi polyhedra");
	...
	_relativeFaceThreshold(0)
{
    ...
    polyhedraVis()->setCullFaces(true);
...
void VoronoiAnalysisModifier::VoronoiAnalysisEngine::perform()
{
	...
	if(_outputPolyhedra) {
		...
		_polyhedraMesh.createFaceProperty(SurfaceMeshFaces::RegionProperty);

		// Create the "Center Particle" region property, which indicates the ID of the particle that is at the center of each Voronoi polyhedron.
		_centerParticleProperty = _polyhedraMesh.createRegionProperty(PropertyStorage::Int64, 1, 0, QStringLiteral("Center Particle"), false).get();

		// Create the "Volume" region property, which stores the volume of each Voronoi cell.
		_cellVolumeProperty = _polyhedraMesh.createRegionProperty(SurfaceMeshRegions::VolumeProperty).get();

		// Create the "Surface Area" region property, which stores the face area of each Voronoi cell.
		_surfaceAreaProperty = _polyhedraMesh.createRegionProperty(SurfaceMeshRegions::SurfaceAreaProperty).get();
	}
		...
		std::vector<int>& voronoiBuffer, std::vector<size_t>& voronoiBufferIndex, QMutex* bondMutex)
	{
		...
		SurfaceMeshData::region_index meshRegionIndex;
		if(_outputPolyhedra) {
			const Point3& center = _positions->getPoint3(index);
			QMutexLocker locker(bondMutex);
		    meshRegionIndex = _polyhedraMesh.createRegion();
			if(_particleIdentifiers)
				_centerParticleProperty->setInt64(meshRegionIndex, _particleIdentifiers->getInt64(index));
			else
				_centerParticleProperty->setInt64(meshRegionIndex, index);
			...
			for(int j = 0; j < v.nu[i]; j++) {
				...
				if(k >= 0) {
					...
					do {
						...
						if(faceAreaThreshold != 0 || _outputPolyhedra) {
