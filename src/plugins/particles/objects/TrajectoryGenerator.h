 ...
 */
class OVITO_PARTICLES_EXPORT TrajectoryGenerator : public StaticSource
{
	Q_OBJECT
	OVITO_CLASS(TrajectoryGenerator)
	Q_CLASSINFO("DisplayName", "Particle trajectories");
	
public:

	/// \brief Constructor.
	Q_INVOKABLE TrajectoryGenerator(DataSet* dataset);
	...
	TrajectoryObject* generateTrajectories(TaskManager& taskManager);
