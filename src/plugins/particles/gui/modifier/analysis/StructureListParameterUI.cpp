...
void StructureListParameterUI::updateStructureCounts()
{
	if(ModifierPropertiesEditor* modifierEditor = dynamic_object_cast<ModifierPropertiesEditor>(editor())) {
		...
		if(DataSeriesObject* series = state.findObject<DataSeriesObject>(QStringLiteral("structures"), modifierEditor->modifierApplication())) {
			_structureCounts = series->getYStorage(state);
			if(_structureCounts->dataType() == PropertyStorage::Int64)
				return;
		}
	}
