...
public:
	...
	/// Sets the data series object to be plotted.
	void setSeries(DataSeriesObject* series, const QVector<DataSeriesProperty*>& properties);

	/// Resets the plot.
	void reset() {
		if(_series.target()) {
			_series.setTarget(nullptr);
			_properties.clear();
			updateDataPlot();
		}
	}
	...
	DataSeriesProperty* lookupProperty(DataSeriesProperty::Type type);
...
private:
	...
	public:
		...
		void setLabels(QStringList labels) { _labels = std::move(labels); }
