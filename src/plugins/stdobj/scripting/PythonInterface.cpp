...
PYBIND11_MODULE(StdObj, m)
{
	...
	auto PlotObject_py = ovito_class<PlotObject, DataObject>(m,
			":Base class: :py:class:`ovito.data.DataObject`\n\n"
			"This data object type stores a series of XY data points. "
			"It is generated by certain modifiers that compute histograms and function plots, e.g. "
			"the :py:class:`~ovito.modifiers.CoordinationNumberModifier` and the :py:class:`~ovito.modifiers.HistogramModifier`. "
			"\n\n",
			// Python class name:
			"PlotData")

		.def_property("title", &PlotObject::title, &PlotObject::setTitle,
				"The title of the plot.")
		
		.def_property_readonly("x", [](PlotObject& p) {
			return buildNumpyArray(p.x(), false, py::cast(p));
		})
		.def_property_readonly("y", [](PlotObject& p) {
